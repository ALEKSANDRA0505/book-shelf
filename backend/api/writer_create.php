...
try {
    ...
    if ($result_status->num_rows === 0) {
        $stmt_status->close();
        send_json_response(404, ['error' => 'Requesting user not found']);
        exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO Writer (name, biography, photo_url) VALUES (?, ?, ?)");
    if (!$stmt) throw new Exception('Failed to prepare statement: ' . $conn->error);
    $stmt->bind_param('sss', $name, $biography, $photo_url);
    if (!$stmt->execute()) {
        $err_no = $conn->errno;
        $err_msg = $stmt->error;
        $stmt->close();
        if ($err_no === 1062) {
            ...
            throw new Exception('Failed to create writer: ' . $err_msg);
        }
        exit;
    }
    $new_writer_id = $conn->insert_id;
    $stmt->close();
    send_json_response(201, ['message' => 'Writer created successfully', 'id_writer' => $new_writer_id]);
} catch (Exception $e) {
    $errorCode = 500;
