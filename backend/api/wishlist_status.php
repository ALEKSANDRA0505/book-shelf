...
$sql = "SELECT id_wishlist_item FROM WishlistItem WHERE id_user = ? AND id_book = ? LIMIT 1";
...
if ($stmt->execute()) {
    $stmt->store_result();
    $isInWishlist = $stmt->num_rows > 0;
    send_json_response(200, ['isInWishlist' => $isInWishlist, 'id_book' => $book_id]);
} else {
    send_json_response(500, ['error' => 'Failed to check wishlist status: ' . $stmt->error]);
}
$stmt->close();
$conn->close();
?> 